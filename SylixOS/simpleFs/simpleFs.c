...
*********************************************************************************************************/
LW_API
INT  API_SimpleFsDrvInstall (VOID)
{
...
*********************************************************************************************************/
LW_API
INT  API_SimpleFsDevCreate (PCHAR   pcName, PLW_BLK_DEV  pblkd)
{
    PRAM_VOLUME     pramfs;
    size_t          stMax;
    ...
    pramfs->RAMFS_hVolLock = API_SemaphoreMCreate("simplevol_lock", LW_PRIO_DEF_CEILING,
...
*********************************************************************************************************/
LW_API
INT  API_SimpleFsDevDelete (PCHAR   pcName)
{
                         ...
                         INT             iMode)
{
    ...
    if(pramnFather)
        printk("after ram open pramnFather:%s\r\n",pramnFather);
    ...
    if (pramn) {
        ...
        pramn = __ram_maken(pramfs, pcName, pramnFather, iMode, LW_NULL);/* bLast=True指的是查找文件所在的文件目录存在，只是没有该文件 */
        if (pramn) {
            bCreate = LW_TRUE;
            goto    __file_open_ok;
